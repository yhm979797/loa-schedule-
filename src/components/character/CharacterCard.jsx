...
export default function CharacterCard({ character, onDelete, onRaidChange, onRaidSelect, readOnly=false }) {
  ...
  return <article className="character-card">
    <div className="character-head">
      <div className="character-summary"><h3 title={character.name}>{character.name}</h3><p>{character.server} · {character.job}</p><span>아이템 레벨 {character.level.toLocaleString()}</span></div>
      <div className="character-actions"><div className="gold-total"><Coins size={15}/>{formatGold(earned)}</div>{!readOnly && onDelete && <button className="icon-button danger" onClick={() => onDelete(character.id)} aria-label={`${character.name} 삭제`}><Trash2 size={17}/></button>}</div>
    </div>
    <div className="raid-list">
      {(character.raids || []).map((raid) => <div className={`raid-row ${raid.done ? "done" : ""}`} key={raid.slotId}>
        ...
        <div className="raid-main">
          {readOnly ? <><strong>{raid.name}</strong><small>{raid.difficulty || "미선택"}</small></> : <select className="raid-select" value={raid.id || ""} onChange={(e) => onRaidSelect(character.id, raid.slotId, e.target.value)}>
            <option value="">레이드 선택</option>
            {available.map((item) => <option key={item.id} value={item.id} disabled={selectedIds.has(item.id) && item.id !== raid.id}>{item.name} · {item.difficulty}</option>)}
          </select>}
        </div>
